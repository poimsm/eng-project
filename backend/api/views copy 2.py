...
from api.models import Word, QuestionActivity, DescribeImageActivity, UserSentence
...
def user_sentences(request):
    if request.method == 'GET':
        userSentences = UserSentence.objects.filter(user__id=test_user_id).order_by('-created')
        serializer = UserSentenceModelSerializer(userSentences, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    if request.method == 'PUT':
        try:
            ...
            serializer = UserSentenceModelSerializer(sentence, data=request.data, partial=True)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_201_CREATED)
            return Response({}, status=status.HTTP_400_BAD_REQUEST)
...
def daily_activities(request):
    ...
    found_questions = combine_unique(
